...
6. **🚫 严禁删除任务**: 未经用户明确授权，**绝对禁止删除任何 vibe-kanban 任务**（包括已完成的历史任务）。任务记录是项目进度的重要历史文档，删除会导致进度追踪丢失。此规则无例外！
...
#### 问题描述
首次启动 VibeHub 服务时遇到多个错误：
1. SQLAlchemy 重复表定义错误
2. API 调用返回 500 错误
3. 数据库表未创建

#### 根本原因
1. **重复模型定义**：`src/storage/models.py` 和 `src/multiplayer/models.py` 中存在重复的 `FriendRequest` 类
2. **代理干扰**：Clash 代理拦截了本地 API 请求
3. **缺少初始化**：`main.py` 中没有调用数据库表创建

#### 解决方案
1. 删除重复的模型定义
2. 设置 NO_PROXY 环境变量：`localhost,127.0.0.1,0.0.0.0`
3. 在 `lifespan` 函数中添加 `db.create_tables()`
...
- 测试日期：2026-02-14
- 测试通过率：9/15 (60%)
- 成功端点：health, player, activity, farm, energy, check-in, market, auctions
- 需修复：achievements, shops, economy, leaderboards 路由前缀

#### 经验教训
- 确保每个数据库表只在一个模型文件中定义
- 开发时设置 NO_PROXY 避免代理干扰
- 应用启动时必须初始化数据库表
- 系统测试应在修复完成后进行
